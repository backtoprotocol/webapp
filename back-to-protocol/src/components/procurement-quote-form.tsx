...
export function ProcurementQuoteForm({ kind }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch("/api/checkup", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerName: form.name, customerEmail: form.email, subject: title, description, service: "Business IT" }) });
